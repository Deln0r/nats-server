...
func TestNRGResetForExtension(t *testing.T) {
	n, cleanup := initSingleMemRaftNode(t)
	defer cleanup()

	// Simulate this node having operated as its own metagroup: WAL entries,
	// extra peers, advanced commit/applied, a snapshot file, and a known
	// leader. ResetForExtension must clear all of it.
	nats0 := "S1Nunr6R" // "nats-0"
	nats1 := "yrzKKRBu" // "nats-1"
	esm := encodeStreamMsgAllowCompress("foo", "_INBOX.foo", nil, nil, 0, 0, true)
	aeMsg1 := encode(t, &appendEntry{leader: nats0, term: 1, commit: 0, pterm: 0, pindex: 0, entries: []*Entry{newEntry(EntryNormal, esm)}})
	...
	// Add a fake peer in addition to ourselves.
	other := nats1
	...
	// State transitions: stepped down to follower, observer mode on,
	// extension state flipped, term/vote cleared so the parent's term wins.
	require_Equal(t, n.State(), Follower)
	require_True(t, n.observer)
